...
public class YukiKanaConverter {
    ...
    static {
        ...
        Iterator var2 = builder.build().entrySet().iterator();

        while(var2.hasNext()) {
            Map.Entry<String, String> entry = (Map.Entry)var2.next();
            String romaji = (String)entry.getKey();
            String hiragana = (String)entry.getValue();
    ...
    private static boolean canStartFromSokuon(String romaji) {
        return !StringUtils.startsWithAny(romaji, new CharSequence[]{"a", "i", "u", "e", "o", "n"});
    }

    public static String conv(String romaji) {
        return containsJapanese(romaji) ? romaji : StringUtils.replaceEach(romaji, ROMAJI_LIST, HIRAGANA_LIST);
    }
    ...
    public static boolean containsJapanese(String text) {
        Pattern pattern = Pattern.compile("[\\p{InCJK_UNIFIED_IDEOGRAPHS}]+");
